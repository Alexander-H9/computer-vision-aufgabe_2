...
while True:
    ...
    if k == ord('q'):
        ...
        horizontal = cv.filter2D(img, -1 , horizontal_kernel)
        img = horizontal.copy()
        ...
        # Wenn man Ableitungs Filter benutzt ist es wichtig zuerst das Signal zu glätten
        #blurred = blur(img, gauss_kernel)
        vertikal = cv.filter2D(img, -1 , vertikal_kernel)
        img = vertikal.copy()
        ...
        print("threshold")
        img_grey = cv.imread("pirates1.jpg", 0) # recommended is to use a grayscale image
        ret, thrashold = cv.threshold(img_grey, 200, 255, cv.THRESH_BINARY)   # https://docs.opencv.org/master/d7/d4d/tutorial_py_thresholding.html
